...
const EventList = async ({ searchParams }: { searchParams: SearchParams }) => {
  ...
  if (params.min_price)
    queryParts.push(`min_price=${encodeURIComponent(params.min_price)}`);
  if (params.max_price)
    queryParts.push(`max_price=${encodeURIComponent(params.max_price)}`);
  if (params.location)
    queryParts.push(`location=${encodeURIComponent(params.location)}`);
  if (params.ordering)
    queryParts.push(`ordering=${encodeURIComponent(params.ordering)}`);
  if (params.is_multi_day)
    queryParts.push(`is_multi_day=${encodeURIComponent(params.is_multi_day)}`);
      ...
      {events.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 ">
          ...
          <p className="text-gray-500 mt-2 text-center max-w-md">
            We couldn't find any events matching your filters. Try adjusting
